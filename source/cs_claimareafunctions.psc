...
;		int HoldingTaken = 0
;		int index = 0
;		while index < CommanderMarkers.Length && HoldingTaken == 0
;			If ConvertCSToLoc(Holdings[i].GetLocation()) == CommanderHoldings[index].GetLocation()
;				HoldingTaken = 1
;			EndIf
;			index += 1
;		endwhile
;		If HoldingTaken == 0
;			CommanderHoldings[ArmyNum].ForceLocationTo(ConvertCSToLoc(Holdings[i].GetLocation()))
;			FindBannerHolding.ForceLocationTo(ConvertCSToLoc(Holdings[i].GetLocation()))
