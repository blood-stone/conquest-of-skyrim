...
Scriptname CS_DefendVillageWhiterunScript Extends Quest Hidden
...
debug.notification(AttackerCount.GetValue())
...
while i < AllLocations.GetSize()
	...
	If Alias_Village.GetLocation() == VillageLoc
		If (AllLocations.GetAt(3) as Location).GetKeywordData(CWOwner) == 1
			VillageHolding.SetValue(11)
			VillageLoc.SetKeywordData(CWOwner, 1)
			ImpMarker.enable()
		ElseIf (AllLocations.GetAt(3) as Location).GetKeywordData(CWOwner) == 2
			VillageHolding.SetValue(12)
			VillageLoc.SetKeywordData(CWOwner, 2)
			SonsMarker.enable()
		ElseIf (AllLocations.GetAt(3) as Location).GetKeywordData(CWOwner) == 70
...
;BEGIN CODE
debug.notification("Troop Death = " + TroopDeath.GetValue())
debug.notification("Attacker Count = " + AttackerCount.GetValue())
