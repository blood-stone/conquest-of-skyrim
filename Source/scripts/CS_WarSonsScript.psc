...
Scriptname CS_WarSonsScript Extends Quest Hidden
...
If Alias_AttackedCity.GetLocation() == Alias_Capital.GetLocation()
	...
	while capitalcitynum < CapitalPriorityLocations.GetSize()
		if (CapitalPriority.GetAt(capitalcitynum) as GlobalVariable).GetValue() == 12
...
Function Fragment_0()
;BEGIN AUTOCAST TYPE CS_WarSonsAttack
Quest __temp = self as Quest
CS_WarSonsAttack kmyQuest = __temp as CS_WarSonsAttack
;END AUTOCAST
;BEGIN CODE
...
(EconReport as CS_EconomyMenuFunctions).CancelMetalImports(2)
(EconReport as CS_EconomyMenuFunctions).CancelMetalExports(2)
(EconReport as CS_EconomyMenuFunctions).CancelWoodImports(2)
(EconReport as CS_EconomyMenuFunctions).CancelWoodExports(2)
(EconReport as CS_EconomyMenuFunctions).CancelFoodImports(2)
(EconReport as CS_EconomyMenuFunctions).CancelFoodExports(2)
...
while citynum < AllGarrisonMarkers.GetSize()
	...
	If Marker.IsEnabled() == 1 || (AllLocations.GetAt(citynum) as Location).GetKeywordData(CWOwner) == 2
		(AllLocations.GetAt(citynum) as Location).SetKeywordData(CWOwner, 2)
		...
		if (FortHoldings.GetAt(citynum) as GlobalVariable).GetValue() != 1
			(AllFortLocations.GetAt(citynum) as Location).SetKeywordData(CWOwner, 2)
			(FortHoldings.GetAt(citynum) as GlobalVariable).SetValue(12)
	...
	Else
		if (AllFortLocations.GetAt(citynum) as Location).GetKeywordData(CWOwner) == 2
			(FortHoldings.GetAt(citynum) as GlobalVariable).SetValue(12)
			(GarrisonsFortCount.GetAt(citynum) as GlobalVariable).SetValue(20)
			(FortMarkersMonster.GetAt(citynum) as ObjectReference).disable()
			(FortMarkers.GetAt(citynum) as ObjectReference).enable()
...
while citynum < AllGarrisonMarkers.GetSize()
	ObjectReference Marker = AllGarrisonMarkers.GetAt(citynum) as ObjectReference
	ObjectReference DefenderMarker = AllSiegeDefenderMarkers.GetAt(citynum) as ObjectReference
	If (AllLocations.GetAt(citynum) as Location).GetKeywordData(CWOwner) == 2
