				...
				if OutfitNum[4].GetValue() == -1
					if CustomOutfitsGendered[4].GetValue() == 1
						if Troop.GetLeveledActorBase().GetSex() == 0
							if !HasAllForms(Troop, CustomOutfitsMale[4]) || Troop.IsInFaction(NeedOutfitFaction) == 1
								EnsureOutfitFormsEquipped(Troop, CustomOutfitsMale[4])
								Troop.RemoveFromFaction(NeedOutfitFaction)
							endif
						elseif Troop.GetLeveledActorBase().GetSex() == 1
							if !HasAllForms(Troop, CustomOutfitsFemale[4]) || Troop.IsInFaction(NeedOutfitFaction) == 1
								EnsureOutfitFormsEquipped(Troop, CustomOutfitsFemale[4])
								Troop.RemoveFromFaction(NeedOutfitFaction)
							endif
						endif
					else
						if !HasAllForms(Troop, CustomOutfits[4]) || Troop.IsInFaction(NeedOutfitFaction) == 1
							EnsureOutfitFormsEquipped(Troop, CustomOutfits[4])
							Troop.RemoveFromFaction(NeedOutfitFaction)
						endif
					endif
				else
	...
	int TroopType = BaseTroop.Find(Troop.GetActorBase())
		...
		if TroopType >=0 && TroopType != 5
				if OutfitNum[TroopType].GetValue() == -1
					if CustomOutfitsGendered[TroopType].GetValue() == 1
						if Troop.GetLeveledActorBase().GetSex() == 0
							if !HasAllForms(Troop, CustomOutfitsMale[TroopType]) || Troop.IsInFaction(NeedOutfitFaction) == 1
								EnsureOutfitFormsEquipped(Troop, CustomOutfitsMale[TroopType])
								Troop.RemoveFromFaction(NeedOutfitFaction)
							endif
						elseif Troop.GetLeveledActorBase().GetSex() == 1
							if !HasAllForms(Troop, CustomOutfitsFemale[TroopType]) || Troop.IsInFaction(NeedOutfitFaction) == 1
								EnsureOutfitFormsEquipped(Troop, CustomOutfitsFemale[TroopType])
								Troop.RemoveFromFaction(NeedOutfitFaction)
							endif
						endif
					else
						if !HasAllForms(Troop, CustomOutfits[TroopType]) || Troop.IsInFaction(NeedOutfitFaction) == 1
							EnsureOutfitFormsEquipped(Troop, CustomOutfits[TroopType])
							Troop.RemoveFromFaction(NeedOutfitFaction)
						endif
					endif
				else
...
bool Function HasAllForms(Actor Troop, FormList OutfitList)
	int i = 0
	while i < OutfitList.GetSize()
		Form OutfitForm = OutfitList.GetAt(i)
		if Troop.GetItemCount(OutfitForm) < 1
			return false
		endif
		i += 1
	endwhile
...
Function EnsureOutfitFormsEquipped(Actor Troop, FormList OutfitList)
	int outfitpiece = 0
	while outfitpiece < OutfitList.GetSize()
		Form OutfitForm = OutfitList.GetAt(outfitpiece)
		Armor OutfitArmor = OutfitForm as Armor
		if Troop.GetItemCount(OutfitForm) < 1
			Troop.AddItem(OutfitForm, 1)
		endif
		if OutfitArmor != None
			if OutfitArmor.HasKeyword(ArmorHeavy) || OutfitArmor.HasKeyword(ArmorLight) || OutfitArmor.HasKeyword(ArmorClothing) || OutfitArmor.HasKeyword(ArmorJewelry)
				Troop.EquipItem(OutfitArmor, true)
			endif
		endif
		outfitpiece += 1
	endwhile
