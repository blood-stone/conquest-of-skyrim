...
Scriptname CS__QF__0531A470 Extends Quest Hidden
...
while citynum < AllGarrisonMarkers.GetSize()
	...
	If (AllLocations.GetAt(citynum) as Location).GetKeywordData(CWOwner) == 1
		(GovQuest as CS_EnemyCities).EnemyCities[citynum].Clear()
...
while citynum < AllGarrisonMarkers.GetSize()
	...
	If (AllLocations.GetAt(citynum) as Location).GetKeywordData(CWOwner) == 70
		...
		(GovQuest as CS_EnemyCities).EnemyCities[citynum].ForceLocationTo(AllLocations.GetAt(citynum) as Location)
		if citynum == 0 || citynum == 1 || citynum == 3 || citynum == 4
