...
ReferenceAlias[] Property KidnapTargets Auto
...
Keyword Property CS_StoryKidnapping Auto
...
Message Property IntelReportMissionKidnapMessage Auto
...
Faction Property KidnappedFaction Auto
...
Function GetKidnapTargets(int citynum)
    if (AllHoldings.GetAt(citynum) as GlobalVariable).GetValue() == 11
        if !(GovPositions.JarlsImp[citynum] as Actor).IsInFaction(KidnappedFaction)
            KidnapTargets[0].ForceRefTo(GovPositions.JarlsImp[citynum])
        endif
        
        if (GovPositions.StewardsImp[citynum] as Actor) && !(GovPositions.StewardsImp[citynum] as Actor).IsInFaction(KidnappedFaction)
            KidnapTargets[1].ForceRefTo(GovPositions.StewardsImp[citynum])
        endif
        
        if (GovPositions.HousecarlsImp[citynum] as Actor) && !(GovPositions.HousecarlsImp[citynum] as Actor).IsInFaction(KidnappedFaction)
            KidnapTargets[2].ForceRefTo(GovPositions.HousecarlsImp[citynum])
        endif
...
Function MenuCityMissionKidnap(int citynum, int Response = 0)
    Location IntelCityLoc = IntelCity.GetLocation()
    Location IntelCityVanilla = AllLocations.GetAt(citynum) as Location
    GetKidnapTargets(citynum)
    Response = IntelReportMissionKidnapMessage.show()
    if Response < 5 
        IntelCityLoc.SetKeywordData(SpyNetworkMission, 6)
        CS_StoryKidnapping.SendStoryEvent(akLoc = IntelCityVanilla, akRef1 = KidnapTargets[Response].GetActorRef())
    else
        MenuCityMission(citynum)
    endif
EndFunction
