...
Scriptname CS_WarImperialsScript Extends Quest Hidden
...
If Alias_AttackedCity.GetLocation() == Alias_Capital.GetLocation()
	...
	while capitalcitynum < CapitalPriorityLocations.GetSize()
		if (CapitalPriority.GetAt(capitalcitynum) as GlobalVariable).GetValue() == 11
...
while citynum < AllGarrisonMarkers.GetSize()
	...
	If (AllLocations.GetAt(citynum) as Location).GetKeywordData(CWOwner) == 1
...
;BEGIN AUTOCAST TYPE CS_WarImperialsAttack
Quest __temp = self as Quest
CS_WarImperialsAttack kmyQuest = __temp as CS_WarImperialsAttack
;END AUTOCAST
;BEGIN CODE
kmyQuest.UnRegisterForUpdateGameTime()
Capitulation.show()
DefeatedGlobal.SetValue(1)
...
(EconReport as CS_EconomyMenuFunctions).CancelMetalImports(1)
(EconReport as CS_EconomyMenuFunctions).CancelMetalExports(1)
(EconReport as CS_EconomyMenuFunctions).CancelWoodImports(1)
(EconReport as CS_EconomyMenuFunctions).CancelWoodExports(1)
(EconReport as CS_EconomyMenuFunctions).CancelFoodImports(1)
(EconReport as CS_EconomyMenuFunctions).CancelFoodExports(1)
...
while citynum < AllGarrisonMarkers.GetSize()
	ObjectReference Marker = AllGarrisonMarkers.GetAt(citynum) as ObjectReference
	ObjectReference DefenderMarker = AllSiegeDefenderMarkers.GetAt(citynum) as ObjectReference
	If Marker.IsEnabled() == 1 || (AllLocations.GetAt(citynum) as Location).GetKeywordData(CWOwner) == 1
		(AllLocations.GetAt(citynum) as Location).SetKeywordData(CWOwner, 1)
		(AllLocationsCS.GetAt(citynum) as Location).SetKeywordData(Attackable, 1)
		Jarls[citynum].ForceRefTo(JarlsInitial[citynum].GetActorRef())
		if (FortHoldings.GetAt(citynum) as GlobalVariable).GetValue() != 1
			(AllFortLocations.GetAt(citynum) as Location).SetKeywordData(CWOwner, 1)
			(FortHoldings.GetAt(citynum) as GlobalVariable).SetValue(11)
			(GarrisonsFortCount.GetAt(citynum) as GlobalVariable).SetValue(20)
			(FortMarkersMonster.GetAt(citynum) as ObjectReference).disable()
			(FortMarkers.GetAt(citynum) as ObjectReference).enable()
			(FortTriggerBoxes.GetAt(citynum) as ObjectReference).enable()
			FortCount.SetValue(FortCount.GetValue() + 1)
		endif
		if citynum == 0 || citynum == 1 || citynum == 3 || citynum == 4
	...
	Else
		if (AllFortLocations.GetAt(citynum) as Location).GetKeywordData(CWOwner) == 1
			(FortHoldings.GetAt(citynum) as GlobalVariable).SetValue(11)
			(GarrisonsFortCount.GetAt(citynum) as GlobalVariable).SetValue(20)
			(FortMarkersMonster.GetAt(citynum) as ObjectReference).disable()
			(FortMarkers.GetAt(citynum) as ObjectReference).enable()
...
kmyQuest.RegisterForSingleUpdateGameTime(NextSiege)
...
LocationAlias Property DefendCityRebel  Auto  
