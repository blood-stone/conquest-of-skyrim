Scriptname CS_FactionWarBattleCommanderScript extends ReferenceAlias  
...
Function SetTroopOutfit()
    ...
    if MilitaryScript.HasCustomCommanderOutfits[UnitType] == 0
        debug.trace("Commander does not have custom outfit")
        ;/if EquipmentTier >= 0
        ...
        if Troop.GetLeveledActorBase().GetSex() == 0
            TroopOutfit = MilitaryScript.CustomOutfitsCommanderMale.GetAt(UnitType) as Outfit
            TroopItems = MilitaryScript.LeveledItemsCommanderMale.GetAt(UnitType) as LeveledItem
        else
            TroopOutfit = MilitaryScript.CustomOutfitsCommanderFemale.GetAt(UnitType) as Outfit
            TroopItems = MilitaryScript.LeveledItemsCommanderFemale.GetAt(UnitType) as LeveledItem
        endif
    endif
