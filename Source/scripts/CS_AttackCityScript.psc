...
Function InitialSetUp()
    ...
    if (AllHoldings.GetAt(citynum) as GlobalVariable).GetValue() == 11
        ...
        if TMImperial.Capital == citynum && (TMImperial.CityCount + TMImperial.MinorCityCount) > 1
            TMImperial.DisableLeaders()
        elseif TMImperial.Capital == citynum && (TMImperial.CityCount + TMImperial.MinorCityCount) == 1
    ...
    elseif (AllHoldings.GetAt(citynum) as GlobalVariable).GetValue() == 12
        ...
        ;debug.notification("Sons Capital = " + TMSons.TMData.GetKeywordData(TMSons.Capital)+ ", CityCounts = " +  (TMSons.CityCount + TMSons.MinorCityCount) + " citynum = " + citynum)
        if (TMSons.CityCount + TMSons.MinorCityCount) > 1 && citynum == 4
            ;debug.notification("Sons Capital = " + citynum + ", CityCounts = " +  (TMSons.CityCount + TMSons.MinorCityCount) + " citynum = " + citynum)
            ;debug.notification("Giving Jarlship to Steward")
            TMSons.DisableLeaders()
            Jarl.ForceRefTo(GovPositions.StewardsSons[citynum])
            Housecarl.Clear()
        elseif TMSons.Capital == citynum && (TMSons.CityCount + TMSons.MinorCityCount) > 1 && citynum != 4
            TMSons.DisableLeaders()
            ;debug.notification("Sons Capital = " + citynum + ", CityCounts = " +  (TMSons.CityCount + TMSons.MinorCityCount) + " citynum = " + citynum)
            ;debug.notification("Disabling Leaders")
        elseif TMSons.Capital == citynum && (TMSons.CityCount + TMSons.MinorCityCount) == 1
            ;debug.notification("Sons Capital = " + citynum + ", CityCounts = " +  (TMSons.CityCount + TMSons.MinorCityCount) + " citynum = " + citynum)
...
Function JarlJoinBattle()
    ...
    Jarl.GetActorRef().SetRelationshipRank(Game.GetPlayer(), -1)
    ...
    Housecarl.GetActorRef().SetRelationshipRank(Game.GetPlayer(), -1)
