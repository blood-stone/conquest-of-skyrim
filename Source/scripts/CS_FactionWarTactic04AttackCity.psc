Scriptname CS_FactionWarTactic04AttackCity extends ObjectReference  

import CS_CommonFunctions

CS_FactionWarLocationScript CityActivatorScript

Keyword Property PlayerAttackCityKeyword Auto
Keyword Property PlayerDefendCityKeyword Auto
Keyword Property AICityBattleKeyword Auto
...
Function SetUp()
    ...
    CityActivatorScript = BattleSpaceScript.City as CS_FactionWarLocationScript
    
    DesiredTroopCount = 20*CityActivatorScript.DefendingRegiments.Length - 2*(OperationFaction as CS_FactionManagementMilitary).Aggression
EndFunction
...
bool Function CheckRequirementRegiments()
    ...
    if CityActivatorScript.DefendingRegiments.Length > 0
        return true
    else
        return false
    endif
...
Function AttackCity()
    ...
    if OperationFaction == BattleSpaceScript.CS_FactionPlayer
        if TargetFaction == BattleSpaceScript.CS_FactionMajorEmpireSkyrim
            PlayerAttackCityKeyword.SendStoryEvent(akRef1 = BattleSpaceScript.City, akRef2 = self as ObjectReference)
    ...
    elseif TargetFaction == BattleSpaceScript.CS_FactionPlayer
        if OperationFaction == BattleSpaceScript.CS_FactionMajorEmpireSkyrim
            PlayerDefendCityKeyword.SendStoryEvent(akRef1 = BattleSpaceScript.City, akRef2 = self as ObjectReference)
        elseif OperationFaction == BattleSpaceScript.CS_FactionMajorSons
            PlayerDefendCityKeyword.SendStoryEvent(akRef1 = BattleSpaceScript.City, akRef2 = self as ObjectReference)
        endif
    else
        if TargetFaction == BattleSpaceScript.CS_FactionMajorEmpireSkyrim
            ;AICityBattleKeyword.SendStoryEvent(akRef1 = BattleSpaceScript.CityImp, akRef2 = self as ObjectReference)
        elseif TargetFaction == BattleSpaceScript.CS_FactionMajorSons
            ;AICityBattleKeyword.SendStoryEvent(akRef1 = BattleSpaceScript.CitySons, akRef2 = self as ObjectReference)
        endif
        debug.trace((OperationFaction as CS_FactionManagementDiplomacy).FactionNameString + " has attacked " + (BattleSpaceScript.City as CS_CityManagementOwnership).VanillaCity.GetName() + "!")
        debug.notification((OperationFaction as CS_FactionManagementDiplomacy).FactionNameString + " has attacked " + (BattleSpaceScript.City as CS_CityManagementOwnership).VanillaCity.GetName() + "!")
    endif
EndFunction
...
State Launched
    ...
    Event OnUpdate()
        ...
        debug.trace("City Attack completed")
