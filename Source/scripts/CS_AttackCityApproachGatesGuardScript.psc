Scriptname CS_AttackCityApproachGatesGuardScript extends ReferenceAlias  
...
Function SpawnTroop()
    GoToState("ForceWait")
    ObjectReference CityManagementActivator = (GetOwningQuest() as CS_AttackCityApproachGatesScript).CityManagementActivator.GetRef()
    ...
    GuardBase = MilitaryScript.GuardTroopType
    debug.trace("Spawning troop")
    Actor Troop
    if GuardMarker
        Troop = GuardMarker.PlaceAtMe(GuardBase, abForcePersist=true) as Actor
