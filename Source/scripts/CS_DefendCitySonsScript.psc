...
Scriptname CS_DefendCitySonsScript Extends Quest Hidden
...
int i = AllLocations.Find(Alias_City.GetLocation())
	...
	If Alias_City.GetLocation() == CityLoc
		...
		EndIf
		Alias_NewJarl.ForceRefTo(Jarls[i])
		NewJarlMessage.show()
		(WarQuest as CS_ReplaceJarl).ReplaceJarl(i, Jarls[i] as Actor)
...
(LoyaltyQuest as CS_LoyaltyCheckScript).AdjustLoyalty((LoyaltyQuest as CS_LoyaltyCheckScript).ConvertVanillaToCS(Alias_City.GetLocation()), 10)
