...
Function InitializeAllQuests()
    CS_StoryStartUp.SendStoryEventAndWait()
...
Function SetUpCapital(int capitalnum)
    ...
    if MarriageScript.NewSpouseAlias.GetActorRef() && !MarriageScript.NewSpouseAlias.GetActorRef().IsInFaction(MarriageScript.CS_SpouseInteractFaction)
        MarriageScript.SetUpSpouseHouse()
    endif
EndFunction
...
Function ChooseTroopRacesMenu()
    ChooseTroopRaces.StartMenu(SetTroops=true, SetSettlers=false)
EndFunction

Function ChooseSettlerRacesMenu()
    ChooseTroopRaces.StartMenu(SetSettlers=true, SetTroops=false)
EndFunction
...
Function MoveGovToBase()
    if BaseFlag.GetRef().IsInLocation(Capital.GetLocation())
        if Steward.GetRef() && !Steward.GetRef().IsInLocation(Capital.GetLocation()) && !ShouldStayWithPlayer(Steward.GetActorRef())
            Steward.GetRef().MoveTo(BaseFlag.GetRef(), 10, 10, 50)
        endif
        
        if ASIC.GetRef() && !ASIC.GetRef().IsInLocation(Capital.GetLocation()) && !ShouldStayWithPlayer(ASIC.GetActorRef())
            ASIC.GetRef().MoveTo(BaseFlag.GetRef(), 10, 10, 50)
        endif
        
        if Blacksmith.GetRef() && !Blacksmith.GetRef().IsInLocation(Capital.GetLocation()) && !ShouldStayWithPlayer(Blacksmith.GetActorRef())
            Blacksmith.GetRef().MoveTo(BaseFlag.GetRef(), 10, 10, 50)
        endif
        
        if CourtMage.GetRef() && !CourtMage.GetRef().IsInLocation(Capital.GetLocation()) && !ShouldStayWithPlayer(CourtMage.GetActorRef())
            CourtMage.GetRef().MoveTo(BaseFlag.GetRef(), 10, 10, 50)
        endif
    endif
EndFunction

bool Function ShouldStayWithPlayer(Actor GovActor)
    if !GovActor
        return false
    endif
    return GovActor.IsPlayerTeammate()
EndFunction
