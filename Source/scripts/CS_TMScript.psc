...
int Property Aggressiveness Auto
int Property WarPriority Auto
Keyword Property CWOwner Auto
int Property ActiveCommanderCount Auto
int Property TotalCommanderCount Auto
int Property AttackingArmies Auto
int Property Capital Auto

Keyword Property CS_StoryDefendCamp Auto
Keyword Property CS_StoryDefendFort Auto
Keyword Property CS_StoryDefendCity Auto

int Property CityCount Auto
int Property MinorCityCount Auto
int Property FortCount Auto
int Property VillageCount Auto
int Property WarScore Auto
...
int Property EquipmentDeficit Auto
    ...
    while i < 9
        ...
        if (AllLocations.GetAt(i) as Location).GetKeywordData(CWOwner) == (FactionNum - 10) || (AllGarrisonMarkers.GetAt(i) as ObjectReference).IsEnabled() == true
            ...
            if i < 5
                CityCount += 1
                if i == Capital
                    TotalCommanderCount += 10
                else
                    TotalCommanderCount += 6
                endif               
            elseif i >= 5 && i < 9
                MinorCityCount += 1
                TotalCommanderCount += 4
            else
                VillageCount += 1
            endif
...
Function AssignLeader(Actor ForcedLeader = None)
    if !Leader.GetActorRef() || Leader.GetActorRef().IsDead()
        if ForcedLeader
            Leader.ForceRefTo(ForcedLeader)
        else
            int i = 0
            while i < LineOfSuccession.Length
                Actor NextLeader = LineOfSuccession[i]
                ActorBase NextLeaderBase = LineOfSuccession[i].GetActorBase()
                if NextLeaderBase.GetDeadCount() == 0
                    Actor NewNextLeader = NextLeader.PlaceActorAtMe(NextLeaderBase)
                    NextLeader.Disable()
                    Leader.ForceRefTo(NewNextLeader)
                endif
                i += 1
            endwhile
        endif
...
Function FortTakeover()
    ...
    while i < AllLocationsFort.GetSize()
        if (AllLocationsFort.GetAt(i) as Location).GetKeywordData(CWOwner) == (FactionNum - 10)
            ...
            ActiveCommanderCount = TotalCommanderCount + 1
            FortCount += 1
        endif
        i += 1
    endwhile
...
Function DistributeCommandersInitial()
    int UnusedCommanders = (TotalCommanderCount - ActiveCommanderCount) as int
    int i = 0
    ...
    while i < AllLocationsFort.GetSize() && UnusedCommanders > 0
        ...
        i += 1
    endwhile
    ActiveCommanderCount = TotalCommanderCount - UnusedCommanders
EndFunction
...
Function SetCommanderCount()
    ...
    commandercountactive += AttackingArmies
    while i < 9
        if (AllLocations.GetAt(i) as Location).GetKeywordData(CWOwner) == (FactionNum - 10)
        ...
        i += 1
    endwhile
    TotalCommanderCount = commandercount
    ActiveCommanderCount = commandercountactive
EndFunction
...
Function RespawnArmies()
    ...
    int InitialCommanders = ActiveCommanderCount as int
    int UnusedCommanders = (TotalCommanderCount - ActiveCommanderCount) as int
    debug.trace("TotalCommanderCount = " + TotalCommanderCount + ", ActiveCommanderCount = " + ActiveCommanderCount)
    int NewCommanders = CityCount + MinorCityCount
    ...
    EquipmentDeficit += (ActiveCommanderCount as int - InitialCommanders)*20
...
Function DisplayInvasionObjectives(int holdnum)
    ...
    if AtWarPlayer || Defeated
        Quest InvasionQuest = InvasionObjectiveQuests.GetAt(holdnum) as Quest
        bool ShowObjectiveHint = false
        
        if !InvasionQuest.IsRunning()
            InvasionQuest.Start()
        ...
        if !HasLandSuperiority && !HasFort && !HasCity
            ...
            InvasionQuest.SetStage(10)
            ObjectiveQuest.SetObjectiveCompleted(holdnum)
        elseif HasLandSuperiority && HasFort && HasCity && !PlayerCamp
            (AllLocationsCS.GetAt(holdnum) as Location).SetKeywordData(Contested, 0)
            InvasionQuest.SetStage(11)
        else

            if HasLandSuperiority
                if !InvasionQuest.IsObjectiveDisplayed(0) || InvasionQuest.IsObjectiveCompleted(0)
                    ShowObjectiveHint = true
                endif
                InvasionQuest.SetObjectiveCompleted(0, false)
                InvasionQuest.SetObjectiveDisplayed(0, true, true)
            endif

            if HasFort
                if !InvasionQuest.IsObjectiveDisplayed(2) || InvasionQuest.IsObjectiveCompleted(2)
                    ShowObjectiveHint = true
                endif
                InvasionQuest.SetObjectiveCompleted(2, false)
                InvasionQuest.SetObjectiveDisplayed(2, true, true)
            endif

            if HasCity
                if !InvasionQuest.IsObjectiveDisplayed(1) || InvasionQuest.IsObjectiveCompleted(1)
                    ShowObjectiveHint = true
                endif
                InvasionQuest.SetObjectiveCompleted(1, false)
                InvasionQuest.SetObjectiveDisplayed(1, true, true)
            endif
            ...
            if ShowObjectiveHint
                ShowInvasionObjectiveHint(holdnum, HasLandSuperiority, HasCity, HasFort, PlayerCamp)
            endif
...
Function ShowInvasionObjectiveHint(int holdnum, bool HasLandSuperiority, bool HasCity, bool HasFort, bool PlayerCamp)
    string HoldName = (AllHolds.GetAt(holdnum) as Location).GetName()
    string ObjectivesText = ""

    if HasLandSuperiority
        ObjectivesText = "field control"
    endif
    ...
    if !PlayerCamp
        ...
        ObjectivesText += "Your camp in " + HoldName + " has been lost"
    endif

    if ObjectivesText != ""
        debug.notification("War in " + HoldName + ": enemy still controls " + ObjectivesText)
    endif
EndFunction
...
Function GatherArmiesForCampaignIntoHold(int holdnum, bool reinforcements = false)
    ...
    if AttackCampQuest.IsRunning() && (AttackCampQuest as CS_AttackFortCampScript).enemyfactionnum == FactionNum && (AttackCampQuest as CS_AttackFortCampScript).holdnum == holdnum
        ...
        if reinforcements == false
            DesiredArmies = Aggressiveness
        else
            Aggressiveness = 4
            DesiredArmies = Aggressiveness - (AllLocationsCS.GetAt(WarPriority) as Location).GetKeywordData(ArmiesPatrolling) as int
        endif
        ...
        while i < 9 ;Get number of armies available for campaign
            ...
            if HoldLoc.GetKeywordData(LandSuperiority) == FactionNum && i != Capital
        ...
        if AvailableArmies < DesiredArmies
            Location HoldLoc = AllLocationsCS.GetAt(Capital) as Location
            if GetIsHoldLinked(Capital, holdnum)
                if HoldLoc.GetKeywordData(ArmiesPatrolling) > 1
                    if reinforcements == false
                        if HoldLoc.GetKeywordData(ArmiesPatrolling) > 1
                            AvailableArmies += (HoldLoc.GetKeywordData(ArmiesPatrolling) as int) - 1
                        ...
                        if GetIsHoldLinked(Capital, holdnum, directly=true)
                            if HoldLoc.GetKeywordData(ArmiesPatrolling) > 1
                                AvailableArmies += (HoldLoc.GetKeywordData(ArmiesPatrolling) as int) - 1
                            endif
                        endif
        ...
        int newholdnum = 0
        while AttackingArmies < DesiredArmies && AvailableArmies > 0
            debug.trace("newholdnum = " + newholdnum)
            Location HoldLoc = AllLocationsCS.GetAt(newholdnum) as Location
            if HoldLoc.GetKeywordData(LandSuperiority) == FactionNum && newholdnum != Capital && HoldLoc.GetKeywordData(ArmiesPatrolling) > 1
                if GetIsHoldLinked(newholdnum, holdnum)
                    if reinforcements == false
                        HoldLoc.SetKeywordData(ArmiesPatrolling, HoldLoc.GetKeywordData(ArmiesPatrolling) - 1)
                        AttackingArmies += 1
                        AvailableArmies -= 1
                        debug.trace("Attacking Armies = " + AttackingArmies)
        ...
        debug.trace("Armies gathered: " + (AttackingArmies))
            
        if reinforcements == true
             (AllLocationsCS.GetAt(WarPriority) as Location).SetKeywordData(ArmiesPatrolling, (AllLocationsCS.GetAt(WarPriority) as Location).GetKeywordData(ArmiesPatrolling) + AttackingArmies)
            AttackingArmies = 0
        endif
...
Function CalculateAggressiveness()
    Aggressiveness = 2
    if ActiveCommanderCount < TotalCommanderCount
        Aggressiveness = 0
    else
        int holdpriority = WarPriority
        ...
        if PriorityLoc.GetKeywordData(LandSuperiority) == FactionNum
            ...
            Aggressiveness = CityCount + MinorCityCount
        endif
    endif
...
Function ChooseHoldPriority()
    ...
    int initialpriority = WarPriority
    ...
    if WarPriority == -1 || (WarPriority == TMEnemyScript.WarPriority && EnemyAtWar) || (AllHoldings.GetAt(initialpriority) as GlobalVariable).GetValueInt() == EnemyFactionNum

        int i = 0
        while i < 9
            ChanceToInvade[i] = 0
            Location HoldLoc = AllLocationsCS.GetAt(i) as Location
            if ((EnemyAtWar && i != TMEnemyScript.WarPriority) || !EnemyAtWar) && (AllHoldings.GetAt(i) as GlobalVariable).GetValueInt() != EnemyFactionNum
                ...
                elseif HoldLoc.GetKeywordData(LandSuperiority) == 1 && (CityCount + MinorCityCount == 1) && (AllHoldings.GetAt(i) as GlobalVariable).GetValue() == FactionNum
        ...
        WarPriority = highestholdnum

        debug.trace("Hold Priority set as " + WarPriority)
...
Function LaunchHoldInvasion()
    ...
    if AttackCampQuest.IsRunning() && (AttackCampQuest as CS_AttackFortCampScript).enemyfactionnum == FactionNum && (AttackCampQuest as CS_AttackFortCampScript).holdnum == holdnum
        ...
        if HoldLoc.GetKeywordData(LandSuperiority) != FactionNum && GameDaysPassed.GetValue() >= RecoveryTimes[holdnum]
            GatherArmiesForCampaignIntoHold(WarPriority)
...
Function EstablishLandSuperiority(int holdnum)
    ...
    if FactionNum != 1
        HoldLoc.SetKeywordData(ArmiesPatrolling, AttackingArmies)
        AttackingArmies = 0
        ;ConvertAllFromPatrolToAttack(holdnum)
        (AllGarrisonMarkersCamp.GetAt(holdnum) as ObjectReference).enable()
...
Function LoseLandSuperiority(int holdnum)
    ...
    if WarPriority == holdnum && (AllLocationsFort.GetAt(holdnum) as Location).GetKeywordData(CWOwner) != (FactionNum - 10) && (AllLocations.GetAt(holdnum) as Location).GetKeywordData(CWOwner) != (FactionNum - 10)
...
Function InvadeCity(int holdnum)
    Location HoldLoc = AllLocationsCS.GetAt(holdnum) as Location
    if DefendCampQuest.IsRunning() == 0 && DefendFortQuest.IsRunning() == 0 && DefendCityQuestNew.IsRunning() == 0
        AttackingArmies = HoldLoc.GetKeywordData(ArmiesPatrolling) as int - 1
...
Function AdjustAggressiveness(int increasenum)
    if Aggressiveness + increasenum <= 5
        Aggressiveness = Aggressiveness + increasenum
    endif
EndFunction
