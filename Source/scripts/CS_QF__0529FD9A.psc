...
Scriptname CS_QF__0529FD9A Extends Quest Hidden
...
while i < AllLocations.GetSize()
	...
	If Alias_Village.GetLocation() == VillageLoc
		If (AllLocations.GetAt(1) as Location).GetKeywordData(CWOwner) == 1
			VillageHolding.SetValue(11)
			VillageLoc.SetKeywordData(CWOwner, 1)
			ImpMarker.enable()
		ElseIf (AllLocations.GetAt(1) as Location).GetKeywordData(CWOwner) == 2
			VillageHolding.SetValue(12)
			VillageLoc.SetKeywordData(CWOwner, 2)
			SonsMarker.enable()
		ElseIf (AllLocations.GetAt(1) as Location).GetKeywordData(CWOwner) == 70
